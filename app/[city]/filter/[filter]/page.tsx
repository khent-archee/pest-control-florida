...
export async function generateMetadata({
  ...
  return {
    title: `Best ${getOverrideFilterTitles(processedFilter)} in ${processedCity}, ${DB_NAME} - ${WEBSITE_NAME}`,
    description: `Find the Best ${getOverrideFilterTitles(processedFilter)} in ${processedCity}, ${DB_NAME}`,
    ...(!canBeCrawled && {
      robots: {
        index: false,
        follow: false,
      ...
      {dataList.length >= end && (
        <div className="mt-2 flex flex-1">
          <Button variant="link" asChild className="w-full">
            <Link href={`/${city}/filter/${filter}/2`} passHref>
              {VIEW_MORE_TEXT}
            </Link>
          </Button>
        </div>
  ...
  for (const { city } of uniqueCityStates) {
    ...
    for (const filter of features) {
      ...
      if (directories && directories.length > 0) {
        params.push({ city, filter: filter.key });
      }
    }
  }
